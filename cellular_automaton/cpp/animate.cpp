...
void animate(int gen) {

    World world = readWorld(gen);
    world.cells.push_back(Cell{0, 0, INITIAL_ENERGY, 'n', 6});
    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(event))
            if (event.type == sf::Event::Closed)
                ...
                world = readWorld(gen);
                world.cells.push_back(Cell{0, 0, INITIAL_ENERGY, 'n', 1});
                vector<Food> newFoods = getRandomizedFood();
                world.foods.insert(world.foods.end(), newFoods.begin(), newFoods.end());
                step = 0;
            }
                ...
                FPS = std::min(120.0f, FPS + 5.0f);
            }

            else if (event.type == sf::Event::KeyPressed && event.key.code == sf::Keyboard::Down) {
                FPS = std::max(1.0f, FPS - 5.0f);
            }
                ...
                int dx = mx - middleX;
                int dy = my - middleY;
                int gx = dx >= 0 ? dx / CELL_SIZE : -(((-dx) + CELL_SIZE - 1) / CELL_SIZE);
                int gy = dy >= 0 ? dy / CELL_SIZE : -(((-dy) + CELL_SIZE - 1) / CELL_SIZE);

                bool found = false;
                for (const Cell& c : world.cells) {
                    if (c.x == gx && c.y == gy) {
                        cout << "Clicked Cell at (" << c.x << ", " << c.y << ") energy=" << c.energy << " dir=" << c.energyDir << " memory=" << static_cast<int>(c.memory) << std::endl;
                        found = true;
                        break;
                    }
                }
                if (!found) {
                    for (const Food& f : world.foods) {
                        if (f.x == gx && f.y == gy) {
                            std::cout << "Clicked Food at (" << f.x << ", " << f.y << ") energy=" << f.energy << std::endl;
                            found = true;
                            break;
                        }
                    }
                ...
                if (!found) {
                    std::cout << "Clicked empty cell at (" << gx << ", " << gy << ")" << std::endl;
                }
            }
